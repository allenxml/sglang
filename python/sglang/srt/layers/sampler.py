# ============================================================================
# Sampler 采样器 - 语言模型的"决策轮盘"
# ============================================================================
#
# 【文件功能概述】
# 这个文件实现了 Sampler（采样器），它是推理流水线的最后一环，负责将模型输出的
# logits（原始分数）转换为最终的 token（词元）。
#
# 【理解采样的核心比喻】
# 想象 Sampler 是一个智能轮盘赌盘：
# - 轮盘上每个格子代表词汇表中的一个词（token）
# - 格子的大小由 logits 决定（分数越高，格子越大）
# - 旋转轮盘，指针停在哪个格子，就输出哪个词
# - 不同的采样策略（greedy/top-k/top-p）就像调整轮盘的规则
#
# 【在推理流水线中的位置】
# ModelRunner → Sampler → Token IDs
#    ↓            ↓          ↓
#  logits      采样策略    最终输出
#  (分数)     (轮盘规则)   (选中的词)
#
# 【核心方法概览】
# - forward(): 主入口，执行采样并计算 logprobs
# - _sample_from_probs(): 根据概率分布采样 token
# - _preprocess_logits(): 预处理 logits（应用自定义处理器，检测 NaN）
# - _attach_logprobs_to_output(): 将 logprobs 附加到输出结构
# - compute_logprobs_only(): 只计算 logprobs，不采样（用于评分场景）
#
# ============================================================================
