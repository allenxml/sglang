...
# ==============================================================================
# ====================================================================
# 📚 学习导读：模型运行器（ModelRunner）
# ====================================================================
#
# 【这个文件是做什么的？】
# ModelRunner 是整个推理引擎的"心脏"——它在 GPU 上实际执行
# 大语言模型的前向传播（Forward Pass），是离 GPU 计算最近的代码。
#
# 【生活比喻】
# 如果说 Scheduler 是餐厅经理（决定做什么），
# 那 ModelRunner 就是大厨本人（实际动手炒菜）。
# - forward_extend() = 炒一道新菜（Prefill：处理全部输入）
# - forward_decode() = 给正在炖的菜加一勺料（Decode：生成下一个Token）
# - CUDA Graph = 把常做的菜的步骤录下来，以后照着录像做，更快
#
# 【核心方法】
# - __init__()：加载模型权重到 GPU，分配 KV Cache 显存
# - forward_extend()：Prefill 阶段的前向传播
#   → 一次性处理所有输入 Token，填充 KV Cache
# - forward_decode()：Decode 阶段的前向传播
#   → 每次只处理 1 个新 Token，利用已有的 KV Cache
# - init_cuda_graphs()：预录制 CUDA Graph 以加速 Decode
#
# 【在推理流程中的位置】
# Scheduler → TpWorker → 【ModelRunner】→ GPU 前向传播 → 采样
#
# 【阅读建议】
# 1. 先看 __init__() 了解模型加载过程
# 2. 再看 forward_decode() 了解最常执行的推理路径
# 3. 对性能优化感兴趣可以看 CUDA Graph 相关方法
# ====================================================================
...
# ====================================================================
# 模型运行器类：整个推理引擎中最核心的执行单元
# 它就像工厂里的"生产线"——接收原材料（Token），经过 GPU 加工，
# 产出成品（下一个 Token 的概率分布）。
# 继承 ModelRunnerKVCacheMixin 获得 KV Cache 管理能力。
# ====================================================================
class ModelRunner(ModelRunnerKVCacheMixin):
    """ModelRunner runs the forward passes of the models."""

    # ----------------------------------------------------------------
    # 初始化方法：这是 ModelRunner 的"出生"过程
    # 主要做三件大事：
    # 1. 加载模型权重到 GPU（好比把菜谱和食材准备好）
    # 2. 分配 KV Cache 显存（好比准备好锅碗瓢盆）
    # 3. 录制 CUDA Graph（好比预先排练做菜流程）
    # 参数说明：
    #   - model_config: 模型配置（几层、多少注意力头等）
    #   - mem_fraction_static: 用于 KV Cache 的显存比例
    #   - gpu_id/tp_rank/tp_size: GPU 编号和张量并行信息
    #   - server_args: 服务器启动参数（包含各种开关和配置）
    # ----------------------------------------------------------------
    def __init__(
        self,
    ...
    # ----------------------------------------------------------------
    # 加载模型权重：把模型参数从磁盘搬到 GPU 显存
    # 好比大厨上班前，先把所有食材从仓库搬到厨房。
    # 这一步通常是启动时最耗时的操作（大模型可能几十 GB）。
    # 加载完成后，模型就"住"在 GPU 里，随时可以推理了。
    # ----------------------------------------------------------------
    def load_model(self):
    ...
    # ----------------------------------------------------------------
    # 录制 CUDA Graph（设备计算图）以加速 Decode 阶段
    # CUDA Graph 的原理：把一次完整的 GPU 计算过程"录像"下来，
    # 之后每次推理直接"回放"这段录像，省去了 CPU 反复向 GPU
    # 发送指令的开销。好比把做菜步骤拍成视频，以后照着视频做。
    # 录制会消耗额外显存，但能显著降低 Decode 阶段的延迟。
    # 对不同 batch size 分别录制，覆盖常见的并发请求数量。
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 初始化分段式 CUDA Graph（Piecewise CUDA Graph）
    # 普通 CUDA Graph 只能加速 Decode，而分段式 CUDA Graph
    # 还能加速 Prefill 阶段！它把模型拆成多段分别录制，
    # 在注意力层和 MoE 层的边界处切分，灵活拼接回放。
    # 好比把做菜视频按"切菜→炒菜→装盘"分段录制，
    # 可以根据不同订单灵活组合回放。
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # Decode 阶段的前向传播（推理中最频繁调用的方法！）
    # 每次调用只处理 1 个新 Token，但会利用 KV Cache 中已有的历史信息。
    # 好比大厨每次只往锅里加一勺调料，但锅里已经有之前炖好的底汤。
    # 性能关键路径：生成 100 个 Token 就要调用 ~100 次。
    # 通常会通过 CUDA Graph 加速（避免每次都重新准备 GPU 指令）。
    # ----------------------------------------------------------------
    def forward_decode(
        self,
        forward_batch: ForwardBatch,
    ...
    # ----------------------------------------------------------------
    # Prefill（预填充）阶段的前向传播
    # 一次性处理用户输入的所有 Token（可能几百到几千个），
    # 并把中间结果填充到 KV Cache 中，供后续 Decode 阶段复用。
    # 好比大厨先把一整锅底汤熬好，之后每次只需加一点料。
    # 这个方法每个请求通常只调用一次，但计算量比 Decode 大得多。
    # 返回值包含 can_run_graph 标志，表示是否使用了分段 CUDA Graph。
    # ----------------------------------------------------------------
    def forward_extend(
        self,
        forward_batch: ForwardBatch,
