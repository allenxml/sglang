...
#
# 【这个文件是做什么的？】
# 这是 SGLang 的"前台接待"——用户所有请求都从这里进入。
# 它使用 FastAPI 框架创建一个 HTTP 服务器，提供 RESTful API。
#
# 【生活比喻】
# 就像餐厅的前台：接电话（HTTP请求）、记录点单（解析参数）、
# 把订单传给后厨（TokenizerManager）、把做好的菜端给顾客（返回响应）。
#
# 【工作流程】
# 1. launch_server() 启动 HTTP 服务
# 2. FastAPI 注册各种 API 路由
# 3. 请求到达 → 路由到对应处理函数
# 4. 处理函数调用 TokenizerManager
# 5. 返回结果给用户
#
# 【核心 API 端点】
...
#
# 【阅读建议】
# 1. 先看 launch_server() 了解启动流程
# 2. 再看 v1_chat_completions() 了解主要 API 的处理逻辑
# ====================================================================
"""
...
# --------------------------------------------------------------------
# 🍳 /generate — SGLang 原生生成接口（"后厨直通窗口"）
# --------------------------------------------------------------------
# 这是 SGLang 自己的接口，不走 OpenAI 格式转换，效率最高。
# 就像在餐厅里直接跟厨师说你要什么，省去了服务员翻译菜单的步骤。
#
# 请求流程：
#   客户端 → /generate → TokenizerManager.generate_request()
#         → Scheduler 调度推理 → DetokenizerManager 解码
#         → 流式(SSE)或一次性返回结果
#
# 支持两种模式：
#   - stream=True:  像水龙头一样，token 一个一个流出来（SSE）
#   - stream=False: 等全部生成完毕，一次性端上来
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# 💬 /v1/chat/completions — OpenAI 兼容的对话接口（"最繁忙的窗口"）
# --------------------------------------------------------------------
# 这是使用最多的 API 端点，兼容 OpenAI 的 Chat Completions 格式。
# 就像餐厅里最热门的点餐窗口，大多数顾客（客户端）都走这里。
#
# 请求流程：
#   1. 客户端发送 ChatCompletionRequest（包含 messages 对话历史）
#   2. FastAPI 自动校验 JSON 格式（validate_json_request）
#   3. 交给 OpenAIServingChat.handle_request() 处理
#      → 将 OpenAI 格式转换为内部格式
#      → 调用 TokenizerManager 进行推理
#      → 将结果转换回 OpenAI 格式返回
#
# 这层"翻译"让任何支持 OpenAI API 的客户端都能无缝使用 SGLang。
# --------------------------------------------------------------------
...
# ====================================================================
# 🚀 launch_server — 服务器启动总指挥（"开店仪式"）
# ====================================================================
# 这是整个 HTTP 服务器的启动入口，相当于餐厅的"开店流程"：
#   1. 启动后厨子进程（Scheduler + Detokenizer）
#   2. 初始化前台（TokenizerManager）
#   3. 设置全局状态（菜单、厨师信息等）
#   4. 配置中间件（API 密钥验证、监控指标等）
#   5. 调用 uvicorn.run() 开始监听 HTTP 请求（"开门营业"）
#
# 进程架构：
#   主进程: HTTP Server + TokenizerManager（前台 + 收银台）
#   子进程: Scheduler（调度员）+ DetokenizerManager（出菜口）
#   通信: 通过 ZMQ 进程间通信（像餐厅里的对讲机）
# ====================================================================
def launch_server(
    server_args: ServerArgs,
