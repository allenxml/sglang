...
# ==============================================================================
# ====================================================================
# 📚 学习导读：进程间通信数据结构（IO Struct）
# ====================================================================
#
# 【这个文件是做什么的？】
# 定义了 SGLang 各组件之间传递消息的"信封格式"。
# 当 TokenizerManager 要把请求发给 Scheduler，或 Scheduler 要把结果
# 发回来时，都需要用这里定义的数据结构来"打包"信息。
#
# 【生活比喻】
# 就像快递系统的各种单据：
# - GenerateReqInput = 用户下的订单（包含要问的问题和各种选项）
# - TokenizedGenerateReqInput = 翻译好的订单（文字已转为 Token）
# - BatchTokenIDOutput = 一批做好的结果（GPU 计算完的 Token）
# - BatchStrOutput = 最终的快递包裹（已经把 Token 翻译回人类可读的文字）
#
# 【核心数据结构】
# - GenerateReqInput：用户发来的生成请求（最重要！）
# - TokenizedGenerateReqInput：分词后的请求
# - BatchTokenIDOutput：批次推理结果（Token ID 形式）
# - BatchStrOutput：批次推理结果（字符串形式）
# - FlushCacheReqInput：清空缓存的请求
# - AbortReq：中止请求
#
# 【数据流向】
# 用户请求 → GenerateReqInput → TokenizerManager 分词
#          → TokenizedGenerateReqInput → Scheduler 调度+推理
#          → BatchTokenIDOutput → DetokenizerManager 解码
#          → BatchStrOutput → 返回给用户
#
# 【阅读建议】
# 重点看 GenerateReqInput——它包含了一个推理请求的所有信息，
# 理解了它就理解了 SGLang 请求的"全貌"。
# ====================================================================
"""
...
# --------------------------------------------------------------------------
# 基础请求类（所有请求的"祖先"）
# 就像所有快递单都有一个"运单号"(rid)，BaseReq 给每种请求提供了通用字段。
# --------------------------------------------------------------------------
@dataclass
class BaseReq(ABC):
...
# --------------------------------------------------------------------------
# 批量请求的基类——当多个请求被"打包"成一批处理时使用
# 类比：一次性寄出多个快递，用一个大箱子装着，每个子包裹有自己的运单号
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# 请求计时指标混入类（Mixin）
# 记录一个请求在各阶段花了多长时间，就像快递的物流追踪：
# - queue_time = 在仓库排队等了多久
# - forward_entry_time = 什么时候开始配送（进入前向计算）
# - prefill_launch_delay = 从开始配送到真正上路的等待时间
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# 会话参数——用于多轮对话场景
# 就像你在客服系统中保持一个"对话窗口"，SessionParams 记录了
# 这个窗口的 ID、偏移量等信息，让模型能"记住"之前聊过什么
# --------------------------------------------------------------------------
@dataclass
class SessionParams:
...
# =====================================================================
# ⭐ GenerateReqInput —— 最核心的数据结构！用户发来的生成请求
# =====================================================================
# 这是用户"下订单"时填写的表单，包含了一次推理请求的所有信息：
# - text / input_ids：用户想问的问题（文字或 Token 形式）
# - image_data / video_data / audio_data：多模态输入（图片/视频/音频）
# - sampling_params：采样参数（温度、top_p 等，控制回答的"创造力"）
# - stream：是否流式输出（像打字机一样一个字一个字蹦出来）
# - return_logprob：是否返回概率信息（模型有多"确信"这个回答）
#
# 【生活比喻】
# 就像在餐厅点餐：
# - text = 你要点什么菜（"来一份宫保鸡丁"）
# - sampling_params = 口味偏好（"少盐、微辣"）
# - stream = 是否一道一道上菜（还是全做好一起端上来）
# - image_data = 你拿着图片说"我要这个"
#
# 【注意】这个类既支持单个请求，也支持批量请求（一次点很多菜）
# normalize_batch_and_arguments() 方法负责统一这两种格式
# =====================================================================
@dataclass
...
# =====================================================================
# ⭐ TokenizedGenerateReqInput —— 分词后的生成请求
# =====================================================================
# 经过 TokenizerManager "翻译"后的请求。
# 原来用户写的自然语言文字（text）已经被转换成了模型能理解的
# Token ID 序列（input_ids）。
#
# 【生活比喻】
# 如果 GenerateReqInput 是中文写的订单，
# 那 TokenizedGenerateReqInput 就是翻译成"机器语言"的订单——
# 厨房（GPU）只认识这种格式。
#
# 【与 GenerateReqInput 的关系】
# GenerateReqInput（用户原始请求）
#     ↓ TokenizerManager 进行分词
# TokenizedGenerateReqInput（分词后的请求）
#     ↓ 发送给 Scheduler 进行调度和推理
# =====================================================================
@dataclass
...
# --------------------------------------------------------------------------
# 一批分词后的请求打包在一起，方便批量发送给 Scheduler
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# EmbeddingReqInput —— 嵌入（Embedding）请求
# 不是让模型"生成"文字，而是让模型把文字转换成一个向量（一串数字）。
# 这个向量可以用来做语义搜索、文本相似度计算等任务。
# 类比：不是让厨师做菜，而是让他帮你"品鉴"食材的味道特征。
# --------------------------------------------------------------------------
@dataclass
...
# ⭐ BatchTokenIDOutput —— 批次推理结果（Token ID 形式）
# =====================================================================
# Scheduler（调度器）完成 GPU 推理后，把结果打包成这个格式
# 发送给 DetokenizerManager（解码器）。
#
# 此时结果还是 Token ID（数字），尚未转换回人类可读的文字。
#
# 【生活比喻】
# 厨房做好了菜，但还是半成品状态（Token ID），
# 需要传菜员（DetokenizerManager）摆盘后才能上桌（变成文字）。
#
# 【包含的信息】
# - decoded_texts / decode_ids：增量解码的中间结果
# - finished_reasons：每个请求是否完成（以及为什么完成）
# - prompt_tokens / completion_tokens：统计信息（花了多少 Token）
# - 各种 logprobs 字段：概率信息（模型对每个 Token 有多确信）
# =====================================================================
@dataclass
...
# --------------------------------------------------------------------------
# BatchMultimodalDecodeReq —— 多模态解码请求
# 用于图像生成等多模态任务的解码过程，包含图片分辨率等额外信息。
# --------------------------------------------------------------------------
@dataclass
...
# =====================================================================
# ⭐ BatchStrOutput —— 最终的批次输出结果（字符串形式）
# =====================================================================
# DetokenizerManager 把 Token ID 转换回人类可读的文字后的结果。
# 这是数据流的最后一站，马上就要返回给用户了！
#
# 【生活比喻】
# 菜已经摆好盘、端上桌了——用户看到的就是 output_strs 里的文字。
#
# 【数据流回顾】
# GenerateReqInput → TokenizedGenerateReqInput → BatchTokenIDOutput
#     → BatchStrOutput → 返回给用户
# =====================================================================
@dataclass
...
# --------------------------------------------------------------------------
# BatchEmbeddingOutput —— 嵌入请求的批次输出
# 返回的不是文字，而是向量（一串浮点数），代表文本的"语义指纹"。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# FlushCacheReqInput —— 清空 KV 缓存的请求
# KV Cache 是模型推理时存储中间计算结果的"草稿纸"。
# 清空缓存就像擦掉草稿纸上的内容，释放 GPU 显存。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# PauseGenerationReqInput —— 暂停推理请求
# 让调度器暂时"停下来"，不再处理新的推理任务。
# 支持三种模式：abort（中止所有）、retract（回退等待）、in_place（原地暂停）
# 类比：餐厅暂停接单——可以选择退掉所有菜、暂停出菜、或直接清场。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# ContinueGenerationReqInput —— 恢复推理
# 与 PauseGenerationReqInput 配对，让暂停的调度器重新开始工作。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# UpdateWeightFromDiskReqInput —— 从磁盘更新模型权重
# 在不重启服务的情况下，热加载新的模型权重文件。
# 类比：餐厅不关门，直接换一个新厨师（新模型权重）上岗。
# --------------------------------------------------------------------------
@dataclass
...
# 释放/恢复显存占用——主要用于强化学习（RL）场景。
# 训练时需要显存，就让推理引擎"让出"一部分；训练完再"拿回来"。
# 类比：餐厅白天营业（推理），晚上把场地借给培训班（RL训练）。
# --------------------------------------------------------------------------
@dataclass
...
# 启动/停止性能分析工具（Profiler），帮助开发者找到性能瓶颈。
# 类比：给餐厅装监控摄像头，看看哪个环节最慢。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# OpenSessionReqInput / CloseSessionReqInput —— 会话管理
# 打开/关闭一个多轮对话会话。
# 类比：在客服系统中"开始对话"和"结束对话"。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# LoadLoRAAdapterReqInput —— 动态加载 LoRA 适配器
# LoRA 是一种轻量级微调技术，可以在不修改原始模型的情况下
# "插入"新的能力。就像给手机装一个新 App，不用换手机。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# GetLoadReqInput / GetLoadReqOutput —— 获取服务器负载信息
# 查看当前服务器有多忙：有多少请求在排队、多少在运行、占了多少显存。
# 类比：查看餐厅还有多少桌空位、厨房积压了多少订单。
# --------------------------------------------------------------------------
@dataclass
...
# --------------------------------------------------------------------------
# 安全检查函数：确保所有请求类型都正确继承了 BaseReq 或 BaseBatchReq，
# 并且遵循了命名规范（以 Req / Input / Output 结尾）。
# 这是一个"自检"机制，在模块加载时自动运行，防止开发者忘记继承基类。
# --------------------------------------------------------------------------
