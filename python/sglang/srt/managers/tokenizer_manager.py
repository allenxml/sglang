...
# ==============================================================================
# ====================================================================
# 📚 学习导读：分词管理器（TokenizerManager）
# ====================================================================
#
# 【这个文件是做什么的？】
# TokenizerManager 是请求进入推理系统的"大门"。
# 它负责将用户的文字输入转换为模型能理解的 Token ID 序列，
# 然后通过 ZMQ 消息通道发送给 Scheduler 进行调度。
#
# 【生活比喻】
# 就像餐厅的点单员：
# 1. 听顾客说"我要一份宫保鸡丁"（接收文本请求）
# 2. 把菜名翻译成厨房编号 "#A23"（分词：文字→Token ID）
# 3. 通过对讲机把编号传给后厨（ZMQ 发送给 Scheduler）
# 4. 等后厨做好菜（等待推理结果）
# 5. 把菜端给顾客（返回结果给用户）
#
# 【核心类和方法】
# - TokenizerManager 类：分词管理器的主体
#   - __init__()：初始化分词器、建立 ZMQ 通信
#   - generate_request()：处理一个生成请求的完整流程
#     → 分词 → 发送 → 等待 → 返回
#
# 【在推理流程中的位置】
# HTTP Server → 【TokenizerManager】→ Scheduler → ModelRunner → Detokenizer
#
# 【阅读建议】
# 1. 先看 __init__() 了解初始化过程
# 2. 再看 generate_request() 了解请求处理流程
# ====================================================================
...
# ====================================================================
# 🏗️ TokenizerManager 类：分词管理器的核心
# ====================================================================
# 这是整个文件最重要的类，相当于推理服务的"前台接待"。
# 它继承了两个 Mixin：
#   - TokenizerCommunicatorMixin：提供与其他进程通信的能力
#   - TokenizerManagerMultiItemMixin：处理多项目（如多图片）请求
#
# 整体职责：
#   1. 管理分词器（Tokenizer）和多模态处理器
#   2. 通过 ZMQ 与 Scheduler 和 Detokenizer 通信
#   3. 跟踪每个请求的状态（rid_to_state 字典）
#   4. 处理模型权重更新、LoRA 适配器管理等
# ====================================================================
class TokenizerManager(TokenizerCommunicatorMixin, TokenizerManagerMultiItemMixin):
    """TokenizerManager is a process that tokenizes the text."""

    # ----------------------------------------------------------------
    # 🔧 __init__()：初始化方法 —— 搭建整个"前台"
    # ----------------------------------------------------------------
    # 初始化顺序（就像开餐厅前的准备工作）：
    #   1. 读取模型配置（了解菜单上有什么菜）
    #   2. 初始化分词器（准备好翻译工具）
    ...
    # ----------------------------------------------------------------
    # 📡 init_ipc_channels()：建立 ZMQ 进程间通信通道
    # ----------------------------------------------------------------
    # ZMQ（ZeroMQ）是一个高性能的消息传递库，这里用它来连接
    # 不同的进程。就像在厨房里安装对讲机系统：
    #
    #   recv_from_detokenizer (PULL)：从 Detokenizer 接收结果
    #     → 相当于"出菜口"，后厨做好的菜从这里传出来
    #
    #   send_to_scheduler (PUSH)：向 Scheduler 发送请求
    #     → 相当于"点单传送带"，把顾客的订单传给后厨调度
    #
    # ZMQ 的 PUSH/PULL 模式保证消息不会丢失，且支持异步操作。
    # ----------------------------------------------------------------
    ...
    # 🚀 generate_request()：处理一个生成请求的完整流程
    # ----------------------------------------------------------------
    # 这是最核心的方法！一个请求从进入到返回的完整旅程：
    #
    #   1. 标准化请求参数（检查顾客的点单是否合规）
    #   2. 记录请求日志（在点单本上记一笔）
    #   3. 等待暂停状态解除（如果厨房暂时关闭，先等一下）
    #   4. 获取读锁（确保在处理请求时模型权重不会被更新）
    #   5. 分词处理 _tokenize_one_request()（把文字翻译成 Token ID）
    #   6. 发送请求 _send_one_request()（通过 ZMQ 传给 Scheduler）
    #   7. 等待响应 _wait_one_response()（等后厨做好菜）
    #   8. 通过 yield 返回结果（支持流式输出，一个字一个字地端菜）
    #
    # 注意：这是一个 async generator（异步生成器），
    # 使用 yield 可以实现流式返回，不用等全部生成完才响应。
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 📤 _send_one_request()：通过 ZMQ 将分词后的请求发送给 Scheduler
    # ----------------------------------------------------------------
    # 分词完成后，这个方法负责：
    #   1. 把分词结果序列化并通过 ZMQ 发送给 Scheduler
    #      （相当于把写好的订单通过传送带送到后厨）
    #   2. 创建一个 ReqState 对象来跟踪这个请求的状态
    #      （在点单本上记录这个订单，方便后续查询进度）
    #   3. 把状态存入 rid_to_state 字典中
    #      （rid = request ID，每个请求的唯一身份证号）
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # ⏳ _wait_one_response()：等待并接收一个请求的推理结果
    # ----------------------------------------------------------------
    # 请求发出去后，这个方法负责"守株待兔"：
    #   - 通过 asyncio.Event 等待结果到来
    #     （就像等叫号器响起，表示你的餐做好了）
    #   - 支持流式输出：每生成一部分就 yield 一次
    #     （像火锅店的涮菜，一盘一盘上，不用等全部上齐）
    #   - 非流式输出：等全部完成后一次性返回
    #     （像点的套餐，全部做好后一起端上来）
    #   - 如果客户端断开连接，会自动取消请求（abort）
    #     （顾客走了就不用继续做这道菜了）
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # 🔄 handle_loop()：主事件循环 —— 持续接收 Detokenizer 的结果
    # ----------------------------------------------------------------
    # 这是一个永不停歇的循环，不断地从 Detokenizer 接收推理结果。
    # 就像餐厅前台一直盯着出菜口，菜一出来就分发给对应的顾客。
    #
    # 工作流程：
    #   1. 通过 ZMQ (recv_from_detokenizer) 接收结果
    #   2. 用 _result_dispatcher 根据结果类型分发处理
    #      （不同类型的菜走不同的出餐通道）
    #   3. 更新最后接收时间戳（让看门狗知道系统还活着）
    # ----------------------------------------------------------------
