...
# 这个文件定义了请求调度策略（SchedulePolicy），决定在多个等待请求中，选择哪个请求
# 优先执行，以优化吞吐量、延迟或缓存利用率。
#
# 【生活比喻】Metaphor
# 想象这是一个"医院急诊科分诊系统"：
# - 等待队列（waiting_queue） = 急诊室的候诊患者
# - 调度策略（SchedulePolicy） = 分诊护士的规则
# - FCFS = 先来先服务（公平，但不考虑病情轻重）
# - LOF = 最长输出优先（预计治疗时间最长的先看）
# - LPM = 最长前缀匹配（病史相似的患者优先，复用检查结果）
#
# 【调度策略分类】Policy Categories
#
# 1. 缓存无关策略（Cache-Agnostic）
#    - FCFS（先来先服务）：按到达时间排序，最公平
#    - LOF（最长输出优先）：预计生成最多 token 的请求优先
#    - RANDOM（随机）：随机打乱顺序（用于对比实验）
#    - ROUTING_KEY（路由键）：按路由键频率优先（DP 环境）
#
# 2. 缓存感知策略（Cache-Aware）
#    - LPM（最长前缀匹配）：优先处理能命中更多缓存的请求
#    - DFS_WEIGHT（深度优先权重）：基于 RadixCache 树结构的加权
#
# 【策略对比】Policy Comparison
#
# | 策略 | 吞吐量 | 延迟 | 公平性 | 缓存利用率 | 适用场景 |
# |------|--------|------|--------|-----------|---------|
# | FCFS | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | 通用场景 |
# | LOF | ⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐ | ⭐⭐ | 批处理 |
# | LPM | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 高重复前缀 |
# | DFS_WEIGHT | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | RadixCache 环境 |
#
# 【使用示例】Usage
# 启动服务时指定调度策略：
#   python -m sglang.launch_server \
#     --model meta-llama/Llama-3.1-8B \
#     --schedule-policy lpm \  # 使用最长前缀匹配策略
#     --port 30000
#
# 【关键概念】Key Concepts
# - matched_prefix_len: 请求与缓存的匹配前缀长度（越长越好）
# - in-batch prefix caching: 队列内请求之间也可以共享前缀
# - priority scheduling: 支持请求优先级（VIP 用户优先）
#
# ================================================================================
