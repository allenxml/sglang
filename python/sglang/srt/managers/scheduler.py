...
#
# 【这个文件是做什么的？】
# Scheduler 是 SGLang 的"大脑"——它决定在每一步中，哪些请求应该
# 被送到 GPU 上计算。它管理着请求的排队、分组、执行和完成。
#
# 【生活比喻】
# 就像一个繁忙餐厅的经理：
# - 维护一个等待队列（新来的订单排队）
# - 维护一个运行批次（正在做的菜）
# - 每一轮决定：继续做正在做的菜（Decode），还是开始做新菜（Prefill）
# - 如果后厨（GPU显存）满了，让一些订单等一等
#
# 【核心工作循环】
# Scheduler 运行一个无限循环（event loop），每轮执行：
# 1. 接收新请求 → 加入等待队列
# 2. 获取下一个批次（get_next_batch_to_run）
# 3. 执行批次（调用 TpWorker/ModelRunner）
# 4. 处理结果（采样、完成检查、发送结果）
# 5. 回到步骤 1
#
# 【核心方法】
# - event_loop_normal()：标准事件循环
# - process_input_requests()：处理从 ZMQ 收到的新请求
# - get_next_batch_to_run()：决定下一个批次包含哪些请求
# - run_batch()：执行一个批次的前向传播
#
# 【在推理流程中的位置】
# TokenizerManager → 【Scheduler】→ TpWorker → ModelRunner → Detokenizer
#
# 【阅读建议】
# 1. 先看 __init__() 了解调度器的初始化
# 2. 再看 event_loop_normal() 了解主循环
# 3. 最后看 get_next_batch_to_run() 了解调度决策
# ====================================================================
...
# ---- 调度器类定义 ----
# Scheduler 是整个推理引擎的"指挥中心"，就像交响乐团的指挥：
# 它继承了多个 Mixin 类来获得不同能力（输出处理、权重更新、性能分析、
# 指标统计、分离式推理、多路复用、运行时检查、流水线并行、数据并行注意力等）。
# 所有请求的生命周期——从到达、排队、执行到返回——都由它统一管理。
class Scheduler(
    SchedulerOutputProcessorMixin,
    SchedulerUpdateWeightsMixin,
    ...
    # ---- 标准事件循环 ----
    # 这是调度器的"心跳"，就像心脏不停跳动一样，这个循环永不停止。
    # 每一次循环（每一拍）做三件事：
    #   1. 收信——接收新的推理请求
    #   2. 点菜——选出下一批要计算的请求（get_next_batch_to_run）
    #   3. 上菜——把批次送到 GPU 执行并处理结果
    # 这是最简单的模式：CPU 和 GPU 串行工作，一个忙完另一个才开始。
    @DynamicGradMode()
    ...
    # ---- 重叠事件循环（高级优化版） ----
    # 这是标准循环的"涡轮增压版"。核心思想：当 GPU 在计算当前批次时，
    # CPU 不要闲着，同时去处理上一个批次的结果。
    # 就像一个高效的厨师：左手翻炒当前这道菜（GPU 前向计算），
    # 右手同时把上一道做好的菜装盘（CPU 处理结果）。
    # 这样 CPU 和 GPU 的工作时间互相"重叠"，整体吞吐量更高。
    @DynamicGradMode()
    ...
    # ---- 处理输入请求 ----
    # 这是调度器的"前台接待"，负责处理从 ZMQ 消息队列收到的所有请求。
    # 就像餐厅前台：客人来了（新的推理请求），记下订单送到后厨；
    # 有人取消订单（AbortReq），从队列中移除；
    # 有人问餐厅忙不忙（健康检查），快速回复。
    # 它通过 _request_dispatcher 将不同类型的请求分发给对应的处理函数。
    ...
    # ---- 获取下一个要运行的批次 ----
    # 这是调度器最核心的"决策方法"，就像餐厅经理决定下一轮做哪些菜。
    # 决策逻辑：
    #   1. 先把上一轮完成的 Prefill 请求合并到运行批次中
    #   2. 检查运行批次中的请求是否需要继续 Decode（生成下一个 token）
    #   3. 如果 GPU 显存还有空间，尝试加入新的 Prefill 请求
    #   4. 如果显存不够，可能需要"退回"一些请求（retract），腾出空间
    # 最终返回一个 ScheduleBatch，包含这一轮要送到 GPU 上计算的所有请求。
    # 返回 None 表示当前没有任何请求需要处理（服务器空闲）。
    ...
    # ---- 执行一个批次的前向传播 ----
    # 这是调度器把"订单送进后厨"的关键一步。
    # 它把 ScheduleBatch 转换成 ModelWorkerBatch，然后交给 TpWorker 执行。
    # 就像把整理好的订单（哪些菜、用什么料）递给厨师（GPU），
    # 厨师完成后返回结果（logits/embeddings），再由调度器处理。
    # 对于生成任务，还会进行采样（从概率分布中选出下一个 token）。
    def run_batch(
        self,
        batch: ScheduleBatch,
